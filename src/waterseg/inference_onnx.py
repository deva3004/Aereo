...
def predict_batch(
    ...
    """Mirrors inference.py's predict_batch exactly, swapping the torch forward
    pass for an ONNX Runtime session.run(). Same normalize/pad logic, same
    argmax-over-class-channel logic - only the model call differs.
    """
    padded = []
    for img in imgs:
